...
class GaussianModel:
    ...
    def get_isotropy(self):
        """
        Computes the Isotropy Index for each Gaussian.
        Isotropy Index I = s_min / s_max.
        Returns a tensor of shape [N, 1].
        """
        scales = self.get_scaling
        # 沿最后一个维度（3个尺度）找到最大值和最小值
        s_max, _ = torch.max(scales, dim=1)
        s_min, _ = torch.min(scales, dim=1)
        
        # 计算各向同性度，加上一个极小值避免除以零
        isotropy = s_min / (s_max + 1e-8)
        
        # 返回形状为 [N, 1] 的张量，以便可以像颜色一样被渲染
        return isotropy.unsqueeze(-1)
    
    # [功能保留] 保留你有用的法线计算功能
    @property
    def get_normals(self):
        """
        Computes the normal vector for each Gaussian.
        The normal is aligned with the axis of the smallest singular value (scale).
        """
        scales = self.get_scaling
        smallest_scale_indices = torch.argmin(scales, dim=1)
        # Create a one-hot vector to select the axis
        normals_local = F.one_hot(smallest_scale_indices, num_classes=3).float()
        
        # Get the rotation matrix
        rotations = build_rotation(self.get_rotation)
        
        # Transform the local normal vector to world coordinates
    ...
    def save_ply(self, path):
        ...
        # [功能保留] 虽然get_normals存在，但在保存时仍像官方一样保存零法线，以保持文件格式兼容性
    ...
    def reset_opacity(self):
        """
        Resets the opacity of all Gaussians to a low value.
        This is a core part of the official training schedule to prevent premature convergence.
        """
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity) * 0.01))
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
        print(f"\n[INFO] Performed global opacity reset.")
    ...
    def load_ply(self, path):
        ...
        # 确保SH阶数匹配
        num_sh_features = (self.max_sh_degree + 1) ** 2
        assert len(extra_f_names) == 3 * (num_sh_features - 1)
        
        features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = plydata.elements[0][attr_name]
        features_extra = features_extra.reshape((features_extra.shape[0], 3, num_sh_features - 1))
