...
def readColmapScene(path, images, eval, llffhold=8):
    ...
    if not os.path.exists(ply_path):
        print("Converting point3d.bin to .ply, will happen only once.")
        try:
    ...
    priors_dir = scene_path / "geometry_priors"

    def find_priors(cam_info_list):
        # This part is a bit redundant because CameraInfo is a tuple, we create new ones.
        new_list = []
        for cam in cam_info_list:
            img_basename = Path(cam.image_path).stem
            
            normal_path = priors_dir / f"{img_basename}_normal.png"
            depth_path = priors_dir / f"{img_basename}_depth.png"
            
            new_list.append(cam._replace(
                gt_normal_map_path=str(normal_path) if normal_path.exists() else None,
                gt_depth_map_path=str(depth_path) if depth_path.exists() else None
            ))
        return new_list
    
    train_cam_infos = find_priors(train_cam_infos)
    test_cam_infos = find_priors(test_cam_infos)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(tqdm(frames, desc="Loading Blender JSON")):
...
def readBlenderScene(path, white_background, eval):
    ...
    # Try to load a point cloud if it exists
    try:
        pcd = fetchPly(ply_path)
    except:
        pcd = None
...
# ============================================================================== #
#          >>> [ 最终修复: 将 sceneLoadTypeCallbacks 移到此处 ] <<<                #
# ============================================================================== #
# 这个字典现在定义在它所引用的函数所在的文件中，从而打破了循环导入。
