...
def evaluate_final_model(model_path: str, args: Namespace, gt_pcd_downsampled, alignment_transform: np.ndarray):
    ...
    try:
        ...
        if gt_pcd_downsampled is not None and alignment_transform is not None:
            ...
            print("    -> [INFO] 正在应用从 alignment file 加载的初始变换...")
            initial_transform_inv = np.linalg.inv(alignment_transform)
            pred_pcd_raw.transform(initial_transform_inv)
            
            pred_pcd_down_aligned_coarse = pred_pcd_raw.voxel_down_sample(voxel_size=args.voxel_size)

            final_transformation = align_pcd_with_icp(
                copy.deepcopy(pred_pcd_down_aligned_coarse), gt_pcd_downsampled, 
                threshold=args.icp_threshold,
                initial_transform=np.identity(4)
            )
            
            pred_pcd_raw.transform(final_transformation)
            ...
            print(f"    -> [DEBUG] 用于可视化的最终对齐点云已保存至: {debug_pcd_dir}")
